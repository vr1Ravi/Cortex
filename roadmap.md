...
- [x] **5.6** **LangChain / LangGraph** — chains, orchestration _(DONE)_
- [x] **5.7** **RAG evaluation** — measuring answer quality, faithfulness _(DONE)_
- [x] **5.8** Building an **agent** with tools (function calling loop) _(DONE — PHASE 5 COMPLETE 🎉)_

### Phase 6 — Ship It Like a Pro · 6 sessions

- [x] **6.1** Testing I — pytest, async tests, `httpx` test client _(basics done: pytest+pytest-asyncio, ASGITransport client fixture, health/404/auth-guard tests green)_
- [~] **6.2** Testing II — fixtures, DB test setup, **mocking the LLM** _(SKIPPED for now — test-DB + dependency-override fixtures drafted in conftest.py; LLM-mocking not covered; revisit before deploy)_
- [x] **6.3** **Docker** + docker-compose (app + Postgres + Redis) _(DONE)_
...
| 2026-07-28 | 5.6 LangChain / LCEL | Done. Reframed the hand-built RAG as LangChain: **standard interfaces** (loader/splitter/embeddings/vectorstore/chat model all interchangeable) + **LCEL** composition via `|` where every piece is a **Runnable** (`.invoke/.ainvoke/.stream/.batch`). Scratch `scratch_langchain.py` (deleted): `prompt \| llm \| StrOutputParser()`, felt `.invoke`/`.stream` (streaming for free). Built `app/services/rag_lc.py` `answer_with_rag_lc` **reusing hand-built `retrieve_chunks`** wrapped as `RunnableLambda`: `RunnableParallel(rows, question)` → `RunnablePassthrough.assign(context=_format_docs)` → `.assign(answer=RAG_PROMPT\|llm\|StrOutputParser())` → returns (answer, rows). Added relevance floor via **`RunnableBranch`** (no-rows → canned NOT_FOUND, skips LLM). `POST /rag/ask-lc` (mounted alongside hand-built `/rag/ask`) with citations + inline `[source d:c]`. Deps: `langchain`, `langchain-google-genai` (`ChatGoogleGenerativeAI`). **Takeaway:** LCEL wins for linear compose + streaming + provider-swap; the moment you need branching/carry-state it took RunnableParallel+2×.assign+RunnableBranch+3 lambdas to match ~8 plain lines of hand-built RAG → hand-roll is often clearer. **Bugs (all "bare class vs instance" family):** `StrOutputParser` (→ `TypeError: BaseModel.__init__() takes 1 positional`) and `RunnablePassthrough` (→ `ValidationError func.callable`) both missing `()`; missing `await` on `retrieve_chunks` (→ `'coroutine' object is not iterable`). Note `.assign` is a classmethod (no parens on class). User re-caught "retrieved ≠ used" (citation array = all 5 retrieved, answer used only 1) — gold-standard = parse used `[source]` markers or structured `used_sources`. LangGraph deferred → 5.8 agent. |

| 2026-08-03 | 5.7 RAG evaluation | Done. Concept: RAG has **two failure points — retrieval & generation — measure them SEPARATELY** (else you can't tell which half broke). Scratch `scratch_eval.py` (deleted). **Retrieval eval** (exact, no LLM): labeled EVAL set (question→expected_doc, incl. off-topic expected_doc=None to test the floor); **hit-rate@k** = fraction where correct doc in top-k (also MRR/recall@k concepts). Result: 2/3 = **67%** — the bigger eval set caught a REAL miss ("What is Global?" → wrong doc), vs the earlier trivial 2/2=100%; lesson: doc-level hit-rate is easy on a tiny corpus, short/vague queries embed poorly. **Generation eval** (fuzzy → can't use `==`): **LLM-as-judge** with structured output (`Judgment{faithful,relevant,reason}` via `response_schema` — the 4.3 pattern) grading answer vs retrieved context. Result: **faithful 2/2, relevant 2/2**. **Combined diagnosis:** generation is trustworthy (no hallucination) → the weak link is retrieval → invest in chunking/embeddings/k, NOT the prompt. Caveats: tiny sample (real evals 50–200 Qs); the judge is itself an LLM (spot-check vs human labels; often use a stronger judge model). This is what RAGAS automates (faithfulness/answer-relevance/context-precision). |

| 2026-08-15 | 5.8 Agent with tools (PHASE 5 DONE) | Done. Concept: **chain = fixed pipeline YOU design; agent = LLM decides the flow** (which tool, what args, when done) given a goal + tools. Mechanism = **function-calling loop**: describe tools → model requests a call (it can't run code, only *asks*) → YOUR code executes → feed result back → loop until final answer (cap `MAX_STEPS`). Built raw loop in `app/services/agent.py` `run_agent(session, question, owner_id)`: `types.FunctionDeclaration` (search_documents + list_documents) → `types.Tool`, loop on `response.function_calls`; append model turn (`role=model`) then tool results (`role=user`, `types.Part.from_function_response`). Verified via `scratch_agent.py`: resume Q → calls `search_documents` (model wrote its own query!); "17*23" → NO tool, answers directly; "how many docs" → routes to `list_documents` (tool descriptions are the ONLY routing signal). Then LangGraph 1.2.9 version (`scratch_langgraph_agent.py`): `create_react_agent(llm, tools=[@tool closures])` builds the graph (agent node + tools node + conditional edge + cycle) = **agent as a state machine**; `MessagesState`. Demo did **multi-hop**: list_documents → search_documents → answer (agent PLANNED it). Lesson (like 5.6): understand loop, then use framework; graph adds branching/persistence/human-in-loop/streaming. **Prod concern: agents are call-hungry** (each step = an LLM call → latency+cost), hit free-tier 429 (gemini-3.7-flash = 5 req/min). Note: `create_react_agent` deprecated → `langchain.agents.create_agent` (v1.0). **PHASE 5 COMPLETE (5.1–5.8 + 5.5b).** |

| 2026-08-22 | 6.1 Testing (basics) + 6.3 Docker | 6.1: `pip install pytest pytest-asyncio`; `[tool.pytest.ini_options]` `asyncio_mode="auto"` + `pythonpath="."` (fixes `ModuleNotFoundError: app`). `tests/conftest.py` in-process client fixture (`httpx.AsyncClient` + `ASGITransport(app=app)` — no live server). Green: health 200, unknown route 404, `/documents` no-token → 401/403. Drafted (6.2, SKIPPED) test-DB fixtures: separate `cortex_test` DB, `db_session` (create ext+tables→yield→drop), `test_user`, `auth_client` (overrides `get_db`+`get_current_user` via `app.dependency_overrides`), `monkeypatch` to no-op the Celery `.delay`. **6.3 Docker DONE:** concept image(blueprint)/container(instance)/Dockerfile(recipe)/Compose(multi-container orchestrator). `pip freeze > requirements.txt` (fixed broken single-`=` pins + missing redis/pypdf/langgraph). `Dockerfile` (python:3.14-slim, build-essential, deps-layer-before-code for cache, uvicorn 0.0.0.0:8000). `.dockerignore`. `docker compose up --build` runs 4 services: db (healthcheck pg_isready) → redis → api (`alembic upgrade head` then uvicorn) → worker (celery). **Container networking:** services reach each other by SERVICE NAME (`db:5432`, `redis:6379`) not localhost; `env_file: .env` + `environment:` overrides DATABASE_URL/REDIS_URL/CELERY_* to service names. Verified all 4 Up (db healthy), /health works. Gotchas: collation warning = cosmetic (REFRESH COLLATION VERSION to silence); use `up -d` (detached) to avoid log spam; Compose auto-names images `<project>-<service>` = `fastapi-api`/`fastapi-worker` (redundant manual `cortex-api` can be `docker rmi`'d). Images live in Docker daemon store (Desktop Linux VM `/var/lib/docker`), layers deduped. |
